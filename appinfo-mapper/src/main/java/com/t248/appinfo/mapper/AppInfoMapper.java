...
public interface AppInfoMapper extends MyMapper<AppInfo> {
    ...
    List<CategoryDTO> getAppInfoCategory(@Param("parentId") Long parentId,@Param("cateLevel") String cateLevel);

    List<DataDictionary>  getFlaform(@Param("typeCode") String typeCode);


     List<CategoryDTO> getAllCategory(@Param("parentId") Long parentId,@Param("cateLevel") Integer cateLevel);

     AppinfoVO findOne(QueryParam appinfo);
}